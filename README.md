...
This challenge examines the performance and memory usage of large shards. It indexed data into a single shard index ~25GB at a time and runs up to a shard size of ~300GB. After every 25GB that has been indexed, select index statistics are recorded and a number of simulated Kibana dashboards are run against the index. Two indiuces are created and benchmarked, one with document IDs generated by Elasticsearch and one with application generated UUIDs used as document IDs.
...
| `query_iterations` | Number of timnes each dashboard is simulated at each level | `int` | `10` |
